...
from model import DistMult, Complex,ED_SimplE
...
def main(args, model_path):
    ...
    if args.model is None:
        model =ED_SimplE(args, vocab['e1'].num_token, vocab['rel'].num_token)
    elif args.model == 'ed_simple':
        model = ED_SimplE(args, vocab['e1'].num_token, vocab['rel'].num_token)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='FB15k-237', help='Dataset to use: {FB15k-237, WN18RR')
    parser.add_argument('--l2', type=float, default=0.0, help='Weight decay value to use in the optimizer. Default: 0.0')
    parser.add_argument('--model', type=str, default=None, help='Choose from: {ed_simple, distmult, complex}')
